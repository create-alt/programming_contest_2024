...
class transition():
  ...
  def __init__(self, board, cutter, goal, frequ=1, EPISODE_SIZE=1000, get_actions=None):
    ...
    # 左or上詰めで、行・列の一致に報酬を与えるが、もともと一致していると過剰な報酬を与えることになるので、事前に一致している行・列を格納
    self.h_countforrew, self.w_countforrew = [], []
  ...
  def step(self, act):
    ...
    next_state = self.action(
        state, [act[0], act[1]], self.cutter[act[2]], act[3])
    ...
    if act[0] >= self.state_shape[0]:
      act[0] = self.state_shape[0] - 1

    if act[1] >= self.state_shape[1]:
      act[1] = self.state_shape[1] - 1
  ...
  def action(self, board, pos, cutter, direction, batch_size=1):
    ...
    X_SIZE, Y_SIZE = len(cutter[0]), len(cutter)
    X, Y = int(pos[0]), int(pos[1])
    x_boardsize, y_boardsize = len(board[0]), len(board)
    ...
    if direction in [0, 1]:  # 上下方向の移動
      for j in range(X, X + X_SIZE - 1):
        if 0 <= j < y_boardsize:
          count = 0
          if direction == 0:  # 上方向
            for i in range(Y, y_boardsize):
              if board[i][j] == -1:
                count += 1
              elif count > 0:
            ...
            for i in range(min(Y + Y_SIZE - 1, y_boardsize - 1), -1, -1):
              if board[j][i] == -1:
                count += 1
              elif count > 0:
      ...
      for i in range(Y, min(Y + Y_SIZE, y_boardsize)):

        if 0 <= i < x_boardsize:
          count = 0
          if direction == 2:  # 左方向
            for j in range(X, x_boardsize):
              if board[i][j] == -1:
                count += 1
              elif count > 0:
            ...
            for j in range(min(X + X_SIZE - 1, x_boardsize - 1), -1, -1):
              if board[i][j] == -1:
                count += 1
              elif count > 0:
                board[i][j + count] = board[i][j]
    ...
    for i in range(X_SIZE):
      for j in range(Y_SIZE):
        if cut_index < len(cut_val):
          # direction (int)  => 型抜き後移動する方向(0:上 1:下 2:左 3:右)
          if direction == 0:
            if 0 <= x_boardsize - X_SIZE + i < x_boardsize and 0 <= Y + j < y_boardsize and board[Y + j][x_boardsize - X_SIZE + i] == -1:
              board[Y + j][x_boardsize -
                           X_SIZE + i] = cut_val[cut_index]
              cut_index += 1

          elif direction == 1:
            if 0 <= i < x_boardsize and 0 <= Y + j < y_boardsize and board[Y + j][i] == -1:
              board[Y + j][i] = cut_val[cut_index]
              cut_index += 1

          elif direction == 2:
            if 0 <= X + i < x_boardsize and 0 <= y_boardsize - Y_SIZE + j < y_boardsize and board[y_boardsize - Y_SIZE + j][X + i] == -1:
              board[y_boardsize - Y_SIZE +
                    j][X + i] = cut_val[cut_index]
              cut_index += 1

          elif direction == 3:
            if 0 <= X + i < x_boardsize and 0 <= j < y_boardsize and board[j][X + i] == -1:
